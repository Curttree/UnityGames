...
public class PIDI_2DReflection : MonoBehaviour {
	...
	public Color refColor = Color.clear;
	...
	void OnWillRenderObject () {
		...
		if ( myCam ){
			...
			tempCamera.clearFlags = myCam.clearFlags;
	...
	public void SRPUpdate ( Camera cam ) {
		...
		if ( !tempCamera ){
			tempCamera = new GameObject("TempCamera_REF2D_255", typeof(Camera) ).GetComponent<Camera>();
			tempCamera.gameObject.hideFlags = HideFlags.HideInHierarchy|HideFlags.HideAndDontSave;
			tempCamera.enabled = false;
		}
