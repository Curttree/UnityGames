...
public class AnimateSpeakers : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (lvl == -80F) {
			...
			sing = PitchValue / divisor;
			if (sing > scale * startScale.x)
				sing = scale * startScale.x;
			transform.localScale = startScale + new Vector3 (sing, sing, sing);
		}
	}
